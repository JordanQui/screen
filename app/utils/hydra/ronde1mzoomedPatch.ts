...
export const createRonde1mzoomedPatch = (api: HydraApi): HydraPatchController => {
  ...
  const fbAmt1 = () => Math.min(0.12, (0.10 + (L() + M1() + M2()) * 0.18 + Hsoft() * 0.16) * FB_G())
  const fbAmt2 = () => Math.min(0.15, (0.26 + (L() + M1()) * 0.14 + Hsoft() * 0.12) * FB_G())
