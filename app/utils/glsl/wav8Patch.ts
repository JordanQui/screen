...
void main() {
  ...
  vec4 lowL = hcol(hosc(st, fLow,  0.0), bL  * 0.08, bL  * 0.38, bL  * 1.00);
  vec4 m1L  = hcol(hosc(st, fM1,   0.0), bM1 * 1.00, bM1 * 0.08, bM1 * 0.60);
  vec4 m2L  = hcol(hosc(st, fM2,   0.0), bM2 * 0.00, bM2 * 0.92, bM2 * 0.88);
  vec4 hiL  = hcol(hosc(st, fHigh, 0.0), bHi * 0.88, bHi * 0.72, bHi * 1.00);
